...
import { addSource, removeSource, addReference, removeReference, applyChanges } from '../manifest/patch-manifest';
import { updatingProject } from '../messages';
export default async function applyChangeset(project, changeset, options = { __temp__log_patch: true }) {
    ...
    await parallel(changeset.components.changed, component => writeComponent(component.details.path, component), { progress });
    await parallel(changeset.components.added, async (component) => {
        const path = join(project.paths.dir, 'src', component.filename);
        component.details.path = path;
        await writeComponent(path, component);
    }, { progress });
    await parallel(changeset.components.removed, async (component) => {
        await remove(component.details.path);
    }, { progress });
    await updateManifest(project, changeset, options);
    done();
}
async function updateManifest(project, changeset, options = { __temp__log_patch: true }) {
    const changes = [];
...
async function writeComponent(path, component) {
