import { targetImportFailed, targetIsOpen, targetCreateFailed, targetRestoreFailed, targetNotFound } from '../errors';
 ...
 */
export default async function buildTarget(target, info, options = {}) {
    const { project } = info;
 ...
 */
export async function createTarget(project, target) {
 ...
 */
export async function importTarget(target, info, file, options = {}) {
 ...
 */
export async function backupTarget(project, target) {
 ...
 */
export async function restoreTarget(project, target) {
    const backup = join(project.paths.backup, target.filename);
    const file = join(project.paths.build, target.filename);
    if (!(await pathExists(backup)))
        return;
